...
export function createSnapshotsRepository(db: Database.Database) {
  ...
  const insertSlideStmt = db.prepare(`
    INSERT INTO slides
      (id, presentation_id, position, group_id, kind, code_id, title, blocks, html, notes, transition_json, background_json, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
      ...
      db.transaction(() => {
        ...
        for (const slide of slides) {
          insertSlideStmt.run(
            ...
            slide.html ?? "",
            slide.notes ?? "",
            slide.transition ? JSON.stringify(slide.transition) : null,
