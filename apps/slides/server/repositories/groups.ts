...
import type { ApiSlideGroup } from "@llm-intro/api-contract";
...
export function createGroupsRepository(db: Database.Database) {
  const listStmt = db.prepare(
    "SELECT * FROM slide_groups WHERE presentation_id=? ORDER BY position, id",
  );
  const getByIdStmt = db.prepare("SELECT * FROM slide_groups WHERE id=? AND presentation_id=?");
  const maxPositionStmt = db.prepare(
    "SELECT MAX(position) as max_position FROM slide_groups WHERE presentation_id=?",
  );
  ...
  return {
    ...
    update(
      ...
      db.prepare(
        `UPDATE slide_groups SET ${assignments.join(", ")} WHERE id=? AND presentation_id=?`,
      ).run(...values);
