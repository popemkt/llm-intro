...
const CW = 500, CH = 260
const MONTHS = 12
const X_EXT  = 15      // x-axis extends to show prediction zone
const YMAX   = 1000
const MAX_PTS = 16
...
const INIT_PTS: Pt[] = [
  {x:1,y:140},{x:2,y:215},{x:3,y:260},
  {x:5,y:400},{x:7,y:495},{x:9,y:625},
  {x:11,y:715},{x:12,y:805},
]

const randomLine = () => ({
  slope:     (Math.random() - 0.4) * 90,
  intercept: Math.random() * 500 + 50,
})
...
function Intro() {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      {/* Definition — colour-coded, no box */}
      ...
      {/* Three concepts — clean visual flow */}
      <div style={{display:'flex',alignItems:'center',gap:0,userSelect:'none'}}>
        ...
        <svg width={60} height={52}>
          <line x1={4} y1={26} x2={48} y2={26} stroke={T.muted} strokeWidth={1.5}
                strokeDasharray="4 3"/>
        ...
        >
          ...
          <motion.span
            animate={{opacity: hovered==='algo' ? .7 : 0}}
        ...
        >
          ...
          <motion.span
            animate={{opacity: hovered==='model' ? .75 : 0}}
            transition={{duration:.15}}
            style={{position:'absolute',top:'100%',marginTop:2,fontSize:10,
...
function LearnChart({
  ...
  const [predictX,   setPredictX]   = useState(13.5)  // future sample handle
  ...
  const move = useCallback((e: React.PointerEvent<SVGSVGElement>) => {
    const sp = svgP(e)
    if (drag !== null) {
      didDrag.current = true
      setPts(pts.map((p, i) => i === drag
        ? {x: Math.max(0.5, Math.min(MONTHS, unx(sp.x))),
           y: Math.max(30,   Math.min(YMAX-30, uny(sp.y)))}
        : p))
    }
    if (dragPred) {
      setPredictX(Math.max(MONTHS + 0.1, Math.min(X_EXT - 0.1, unx(sp.x))))
    }
  }, [drag, dragPred, svgP, pts, setPts])

  const up = useCallback(() => {
    if (drag !== null && !didDrag.current) setPts(pts.filter((_, i) => i !== drag))
    setDrag(null); setDragPred(false)
  }, [drag, pts, setPts])
  ...
  const learn  = (n = 1) => {
    ...
    setLine(l => {
      let cur = l
      for (let i = 0; i < n; i++) cur = gdLearn(pts, cur.slope, cur.intercept, 10)
      return cur
    })
    setSteps(s => s + n)
  }
  ...
  const clampY = (v: number) => Math.max(-150, Math.min(YMAX+150, v))
  const y0     = clampY(line.intercept)
  const yM     = clampY(line.slope * MONTHS + line.intercept)
  const yExt   = clampY(line.slope * X_EXT  + line.intercept)
  const pyC    = clampY(predictedY)
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <svg ref={svgRef} viewBox={`0 0 ${CW} ${CH}`}
        style={{width:'100%',maxWidth:CW,cursor:drag!==null||dragPred?'grabbing':'crosshair',
                userSelect:'none'}}
        ...
        {MON.map((lbl, i) => (
          <g key={i}>
            <line x1={scx(i+1)} y1={PT} x2={scx(i+1)} y2={CH-PB}
        ...
        <text x={11} y={CH/2} textAnchor="middle" fill={T.textDim}
              fontSize={9} fontFamily="Inter,sans-serif" transform={`rotate(-90,11,${CH/2})`}>$ →</text>
        ...
        {showRes && pts.map((p, i) => (
          <line key={i}
            ...
            x2={scx(p.x)} y2={scy(clampY(line.slope*p.x+line.intercept))}
            stroke="#f97316" strokeWidth={1.5} strokeDasharray="3 2" opacity={.65}/>
        ))}

        {/* model line — solid through data, dashed into future */}
        ...
        {/* prediction sample handle in future zone */}
        {/* dashed vertical */}
        ...
        {/* data points */}
        {pts.map((p, i) => (
          <circle key={i} cx={scx(p.x)} cy={scy(p.y)} r={6}
            fill={T.surface} stroke={T.accent} strokeWidth={1.5}
            style={{cursor:'grab'}} onPointerDown={e => ptDown(e, i)}/>
        ))}
      </svg>

      {/* controls — single row, stable layout */}
      <div style={{display:'flex',alignItems:'center',gap:6,width:'100%',maxWidth:CW,paddingTop:4}}>
        ...
        <div style={{display:'flex',gap:0,flexShrink:0}}>
          <button onClick={() => learn(1)} disabled={pts.length<2}
            style={{padding:'5px 14px',borderRadius:'16px 0 0 16px',
                    ...
                    cursor:pts.length<2?'not-allowed':'pointer',opacity:pts.length<2?.35:1,
                    lineHeight:1}}>
            Learn
          </button>
        ...
        <button onClick={reset}
          style={{padding:'5px 11px',borderRadius:16,flexShrink:0,
                  ...
                  color:T.textDim,fontFamily:'Inter,sans-serif',fontSize:11,
        ...
        {/* stats — fixed widths so nothing shifts */}
        <div style={{display:'flex',alignItems:'center',gap:12,
          ...
          <span style={{whiteSpace:'nowrap'}}>
            Σ <span style={{color:showRes?'#f97316':T.textDim,fontWeight:600}}>
              ${sigma.toFixed(0)}
            </span>
          </span>
          <span style={{whiteSpace:'nowrap'}}>
            err <span style={{fontWeight:600,
                              color:err>300?'#f97316':err>100?Y:T.highlight}}>
              ${err.toFixed(0)}
            </span>
          </span>
          {/* always rendered, opacity-only transition */}
          <motion.span
...
// ─── Part C: tokens ───────────────────────────────────────────────────────────
function TokenPredict({
  pts, sourcePts, contentRef,
}: {
  pts:        Pt[]
  sourcePts:  Pos[]
  ...
  const [targetPts,   setTargetPts]   = useState<Pos[]>([])
  const [arrivedMask, setArrivedMask] = useState<boolean[]>([])
  ...
  useEffect(() => {
    ...
    const tgts = chipRefs.current.slice(0, count).map(el => {
      if (!el) return {x:0,y:0}
      const r = el.getBoundingClientRect()
      return {x: r.left - br.left + r.width/2, y: r.top - br.top + r.height/2}
    })
    setTargetPts(tgts)
    setArrivedMask(sourcePts.length === 0 ? Array(count).fill(true) : Array(count).fill(false))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [count])

  const arrive = useCallback((i: number) => {
    setArrivedMask(prev => { const n=[...prev]; n[i]=true; return n })
  }, [])

  const allArrived = count > 0 && arrivedMask.slice(0, count).every(Boolean)
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      style={{width:'100%',display:'flex',flexDirection:'column',
              alignItems:'center',gap:22,position:'relative'}}>

      <p style={{fontSize:12,color:T.textDim,fontFamily:'Inter,sans-serif',
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:8,justifyContent:'center',
                   maxWidth:600,minHeight:80}}>
        {SENTENCE.map((word, i) => {
          const isCtx  = i < count
          const isPred = i >= count && (i - count) < predicted
          const isLast = i === latestIdx
          const visible = isCtx ? arrivedMask[i] : isPred
          return (
            <span key={i}
              ref={isCtx ? el => { chipRefs.current[i] = el } : undefined}
              style={{
                display:'inline-flex',alignItems:'center',
                padding:'5px 11px',borderRadius:8,
                fontFamily:'JetBrains Mono,monospace',fontSize:13,fontWeight:500,
                background: isLast ? YBG   : isCtx ? T.surface : `${T.accent}10`,
                border:`1px solid ${isLast ? YB : isCtx ? T.border : `${T.accent}30`}`,
                color: isLast ? Y : isCtx ? T.text : T.accent,
                opacity: visible ? 1 : (isCtx ? 0 : .1),
                transition: isCtx ? 'opacity .3s' : 'opacity .4s, transform .3s',
      ...
      {/* flying circles (SVG in contentRef space) */}
      {targetPts.length > 0 && sourcePts.length > 0 && (
        <svg style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',
                     pointerEvents:'none',overflow:'visible'}}>
          {sourcePts.slice(0, count).map((src, i) => {
            const tgt = targetPts[i]
            if (!tgt || arrivedMask[i]) return null
            return (
              <motion.circle key={i}
                cx={src.x} cy={src.y} r={6}
                fill={T.surface} stroke={T.accent} strokeWidth={1.5}
                animate={{cx:tgt.x, cy:tgt.y}}
                transition={{duration:2.4, delay:i*0.1, ease:[0.32,0,0.18,1]}}
                onAnimationComplete={() => arrive(i)}/>
            )
          })}
        </svg>
      )}
...
export default function LinearRegression({ isActive: _isActive }: SlideProps) {
  ...
  const [sourcePts, setSourcePts] = useState<Pos[]>([])
  const contentRef = useRef<HTMLDivElement>(null)
  const svgRef     = useRef<SVGSVGElement>(null)

  const goTo = (next: Part) => {
    if (next === 'tokens' && svgRef.current && contentRef.current) {
      const cr = contentRef.current.getBoundingClientRect()
      const sr = svgRef.current.getBoundingClientRect()
      const kx = sr.width / CW, ky = sr.height / CH
      setSourcePts(pts.map(p => ({
        x: sr.left - cr.left + scx(p.x) * kx,
        y: sr.top  - cr.top  + scy(p.y) * ky,
      })))
    }
    setPart(next)
  }
    ...
    <div style={{width:'100%',height:'100%',display:'flex',flexDirection:'column',
      ...
      <div ref={contentRef}
        ...
        <AnimatePresence mode="wait">
          ...
          {part==='learn'  && <LearnChart key="learn" svgRef={svgRef} pts={pts} setPts={setPts}/>}
          {part==='tokens' && (
            <TokenPredict key="tokens" pts={pts} sourcePts={sourcePts} contentRef={contentRef}/>
          )}
        </AnimatePresence>
      </div>
