...
const PROMPT_INDICES = TIMELINE.map((c, i) => (c.kind === "prompt" ? i : -1)).filter((i) => i >= 0);
...
function deriveChat(visibleCount: number): { messages: ChatMsg[]; status: ChatStatus | null } {
  ...
  for (let i = 0; i < visibleCount; i++) {
    ...
    if (c.kind === "prompt") {
      if (buf) {
        messages.push({ role: "assistant", text: buf.trim() });
        buf = "";
      }
    ...
    >
      ...
      >
        ...
        {messages.length === 0 && !status && (
          <div style={{ fontSize: 10, color: T.muted, fontStyle: "italic" }}>(empty)</div>
        )}
        {messages.map((m, i) => (
          <motion.div
            key={`msg-${i}-${m.role}`}
    ...
    >
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 10, flexShrink: 0, minHeight: 0 }}>
        ...
        >
          ...
          >
            ...
            <span style={{ color: T.muted, letterSpacing: "0.12em" }}>← drag to scroll →</span>
          </div>
      ...
      >
        <svg
          ...
          style={{ width: "100%", maxWidth: DIAG_W, overflow: "visible" }}
        >
          <ArrowDefs id="arr" />
          ...
          {showOutputArrow && (
            <>
              ...
              >
                ...
                {lastChip &&
                  (lastChip.kind === "thinking" ||
                    lastChip.kind === "tool" ||
                    lastChip.kind === "output") && (
                    <motion.text
