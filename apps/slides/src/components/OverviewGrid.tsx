...
function ThumbnailCell({
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: slide.id,
    disabled: !sortableEnabled || isRenaming || selectMode,
  });
  const dragHandleProps =
    sortableEnabled && !isRenaming && !selectMode ? { ...attributes, ...listeners } : {};
  ...
  return (
    ...
    >
      <motion.div
        ...
        aria-label={
          selectMode
            ? `${selected ? "Deselect" : "Select"} slide ${index + 1}`
            : `Open slide ${index + 1}: ${slide.title}`
        }
      ...
      >
        ...
        <div className="px-3 py-2 border-t border-(--color-border) flex items-center gap-2">
          <span className="text-xs font-mono text-(--color-text-dim)" style={{ flexShrink: 0 }}>
          ...
          {isRenaming ? (
            ...
            <>
              <span className="text-xs font-medium text-(--color-text) truncate flex-1">
                {slide.title}
              </span>
              {sortableEnabled && (
                <button
...
function AddTile({
  ...
  return (
    ...
    >
      {icon}
      <span
        style={{ fontSize: 11, color: "var(--color-text-dim)", fontFamily: "Inter, sans-serif" }}
      >
        {label}
      </span>
    </button>
  ...
  const filtered = useMemo(() => {
    ...
    return sections.map((section) => ({
      ...
      slides: section.slides.filter(
        (s) => s.title.toLowerCase().includes(q) || String(s.number).includes(q),
      ),
    }));
  }, [sections, query]);
        ...
        {state.status === "error" && (
          <div className="flex flex-col items-center gap-3">
            ...
            <p
              ...
              style={{ color: "var(--color-text-dim)", margin: 0, lineHeight: 1.5 }}
...
async function exportPresentation(
  ...
  const filename =
    res.headers.get("Content-Disposition")?.match(/filename="(.+)"/)?.[1] ?? "presentation.html";
...
export function OverviewGrid({
  ...
  const [confirmDeleteSlides, setConfirmDeleteSlides] = useState<
  ...
  const selectedDbSlides = selectedSlides.filter(
    (slide): slide is Extract<UnifiedSlide, { kind: "db" }> => slide.kind === "db",
  );
  const breadcrumbSegments = breadcrumbs ?? [{ label: "Home", to: "/" }, { label: title }];
  const commitLabel = exportCommit ? exportCommit.slice(0, 7) : null;

  const startExport = useCallback(
    async (slideIds?: number[], mode: ExportMode = "player") => {
  ...
  const buildLayoutFromBuckets = (b: Buckets): LayoutInput => ({
    ...
    groups: groups.map((g) => ({
      id: g.id,
      slideIds: (b.byGroup.get(g.id) ?? []).map((s) => s.id),
    })),
  });
  ...
  const resolveBucketFromOver = (b: Buckets, overId: string | number): string | null => {
    ...
    for (const [gid, list] of b.byGroup)
      if (list.some((s) => s.id === id)) return groupBucketKey(gid);
    return null;
  };
    ...
    ({ active, over }: DragOverEvent) => {
      ...
      setDragBuckets((prev) => {
        ...
        if (
          ...
          (typeof over.id === "string" && !over.id.startsWith("bucket:"))
        ) {
  ...
  const handleDragEnd = useCallback(
    ({ active, over }: DragEndEvent) => {
      if (readonly || !dragBuckets) {
        setDragBuckets(null);
        return;
      }
      const activeId = Number(active.id);
      if (Number.isNaN(activeId) || !over) {
        setDragBuckets(null);
        return;
      }
  ...
  const moveSlideToGroup = useCallback(
    (slideId: number, targetGroupId: number) => {
      const nextUngrouped = buckets.ungrouped.filter((s) => s.id !== slideId).map((s) => s.id);
      const nextGroups = groups.map((g) => {
        const current = (buckets.byGroup.get(g.id) ?? [])
    ...
    (groupId: number, direction: -1 | 1) => {
      ...
      onLayoutChange({
        ...
        groups: reorderedGroups.map((g) => ({
          id: g.id,
          slideIds: (buckets.byGroup.get(g.id) ?? []).map((s) => s.id),
        })),
      });
    },
                ...
                return (
                  <button
                    onClick={() => {
                      ...
                      groups.forEach((g) => {
                        if (g.collapsed !== next) onUpdateGroup(g.id, { collapsed: next });
                      });
                    }}
        ...
        <div className="flex flex-col gap-6 p-8 mx-auto w-full" style={{ maxWidth: 1600 }}>
          ...
          <div className="flex flex-col gap-3">
            ...
            {(!ungroupedCollapsed || readonly) && (
              <SortableContext
                items={buckets.ungrouped.map((s) => s.id)}
                strategy={rectSortingStrategy}
              >
                <BucketDrop
                  id={UNGROUPED_KEY}
                  empty={buckets.ungrouped.length === 0 && groups.length > 0}
                >
        ...
        {pickForGroupId != null &&
          (() => {
            ...
            const sections: Array<{
              label: string;
              slides: { id: number; title: string; number: number }[];
            }> = [
              {
                label: "Ungrouped",
                slides: buckets.ungrouped.map((s) => ({
                  id: s.id,
                  title: s.title,
                  number: slideNumber(s.id),
                })),
              },
              ...groups
                ...
                .map((g) => ({
                  label: g.title,
                  slides: (buckets.byGroup.get(g.id) ?? []).map((s) => ({
                    id: s.id,
                    title: s.title,
                    number: slideNumber(s.id),
                  })),
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                background: "none",
                border: "none",
                cursor: exportState.status === "exporting" ? "wait" : "pointer",
              }}
            >
              <Download size={13} />
              Export player
            ...
            <button
              ...
              style={{
                ...
                background: "none",
                border: "none",
                cursor: selectedDbSlides.length === 0 ? "not-allowed" : "pointer",
                opacity: selectedDbSlides.length === 0 ? 0.45 : 1,
              }}
            >
              <Trash2 size={13} />
              Delete
