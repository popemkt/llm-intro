...
const appDescription =
  "Create, inspect, edit, theme, group, and present LLM introduction slide decks.";
...
function isFrontendActionRequest(headers: { get(name: string): string | undefined }) {
  return (
    headers.get("x-agent-native-frontend") === "1" ||
    headers.get("x-request-source") === "agent-native-frontend"
  );
}

function callerFromRequest(
  headers: { get(name: string): string | undefined },
  fallback: ActionCaller,
): ActionCaller {
...
function exposedAgentEntries(actions: AppActions) {
  return Object.entries(actions).filter(
    ([, action]) => action.agentTool !== false && action.publicAgent?.expose !== false,
  );
}
  ...
  return {
    ...
    actions: Object.fromEntries(
      Object.entries(actions).map(([name, action]) => [
        name,
        createActionManifestItem(name, action),
      ]),
    ),
  };
}
...
async function handleMcpToolCall(
  ...
  id: unknown,
  paramsBody: unknown,
  res: Parameters<Parameters<ExpressRouter["post"]>[1]>[1],
) {
...
function registerInvokeRoutes(router: ExpressRouter, actions: AppActions) {
  router.post("/invoke", async (req, res) => {
    ...
    await handleInvoke(
      getActionByName(actions, name),
      body.args ?? {},
      callerFromRequest(req, "tool"),
      res,
    );
  });
  ...
  router.post("/invoke/:name", async (req, res) => {
    await handleInvoke(
      getActionByName(actions, req.params.name),
      req.body ?? {},
      callerFromRequest(req, "tool"),
      res,
    );
  ...
  router.all("/:name", async (req, res) => {
    ...
    try {
      const result = await runAction(
        entry,
        parseActionParams(req.method, req.query, req.body),
        callerFromRequest(req, "http"),
      );
      res.json(result);
    } catch (err) {
      const status = errorStatus(err);
