...
const STEPS = [
  ...
  {
    icon: MousePointer2,
    label: "Find upload button",
    detail: "locate element by text / selector",
    color: T.textDim,
  },
  { icon: null, label: "Select file", detail: "pick file from local path", color: T.textDim },
  {
    icon: MousePointer2,
    label: "Click Submit",
    detail: "trigger form submission",
    color: T.textDim,
  },
...
function LoopDiagram({ isActive }: { isActive: boolean }) {
  return (
    <svg viewBox={`0 0 ${DW} ${DH}`} style={{ width: "100%", maxWidth: DW }}>
      ...
      {/* Claude → Playwright */}
      <FlowArrow
        d={`M${cr(claud)} ${cy(claud)} L${cl(play)} ${cy(play)}`}
        delay={0.2}
        isActive={isActive}
        accent
        markerEnd="url(#darr-a)"
      />
      {/* Playwright → Browser */}
      <FlowArrow
        d={`M${cr(play)} ${cy(play)} L${cl(brow)} ${cy(brow)}`}
        delay={0.4}
        isActive={isActive}
        markerEnd="url(#darr)"
      />
      {/* Browser → Screenshot */}
      <FlowArrow
        d={`M${cx(brow)} ${cb(brow)} L${cx(screen)} ${ct(screen)}`}
        delay={0.6}
        isActive={isActive}
        markerEnd="url(#darr)"
      />
    ...
    >
                    ...
                    <div>
                      <span
                        style={{
                          fontSize: 12,
                          fontWeight: 600,
                          color: i === STEPS.length - 1 ? T.accent : T.text,
                          fontFamily: "Inter, sans-serif",
                        }}
                      >
                        {step.label}
      ...
      >
        <span style={{ fontSize: 10, color: T.textDim, fontFamily: "Inter, sans-serif" }}>
          ↳ This whole workflow becomes a <strong style={{ color: T.accent }}>Skill</strong> — one{" "}
          <code style={{ fontFamily: "JetBrains Mono, monospace", color: T.highlight }}>.md</code>{" "}
          file telling Claude exactly what to do
        </span>
      </motion.div>
    </div>
  );
