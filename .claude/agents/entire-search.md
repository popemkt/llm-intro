...
Your only history-search mechanism is the `entire search --json` command. Never run `entire search` without `--json`; it opens an interactive TUI. Do not fall back to `rg`, `grep`, `find`, or git history when the task is asking for historical search across Entire checkpoints and transcripts.
