...
function Intro() {
  ...
  return (
    ...
    >
      ...
      >
        An <strong style={{ color: T.textDim }}>algorithm</strong> to produce a{" "}
        <strong style={{ color: Y }}>model</strong>, to predict linear trends of{" "}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 0, userSelect: "none" }}>
        ...
        >
          <svg width={70} height={52} style={{ overflow: "visible" }}>
            {[
              ...
              <circle key={i} cx={cx} cy={cy} r={5} fill={T.accent} opacity={0.8} />
            ))}
          </svg>
          <span
            style={{
        ...
        >
          <svg width={70} height={52} style={{ overflow: "visible" }}>
            ...
            {[0, 0.33, 0.66, 1].map((t) => {
              const x = 6 + t * 58,
                y = 10 + t ** 2 * 34 + (1 - t) * t * -4;
              return (
                <circle key={t} cx={x} cy={y} r={3} fill={T.textDim} opacity={0.5 + t * 0.5} />
              );
            })}
          </svg>
          <span
          ...
          <motion.span
            animate={{ opacity: hovered === "algo" ? 0.7 : 0 }}
        ...
        >
          <svg width={70} height={52} style={{ overflow: "visible" }}>
            {[
              ...
              <circle key={i} cx={cx} cy={cy} r={4} fill={T.accent} opacity={0.45} />
            ))}
          ...
          <motion.span
            animate={{ opacity: hovered === "model" ? 0.75 : 0 }}
            transition={{ duration: 0.15 }}
            style={{
              position: "absolute",
              top: "100%",
...
function LearnChart({
  ...
  const learn = (n = 1) => {
    ...
    setLine((l) => {
      let c = l;
      for (let i = 0; i < n; i++) c = gdLearn(pts, c.slope, c.intercept, 10);
      return c;
    });
    setSteps((s) => s + n);
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ width: "100%", maxWidth: CW }}>
        ...
        >
          ...
          <rect
            x={scx(MONTHS)}
            y={PT}
            width={scx(X_EXT) - scx(MONTHS)}
            height={CH - PT - PB}
            fill={`${Y}07`}
          />
          <text
            x={(scx(MONTHS) + scx(X_EXT)) / 2}
            y={PT + 11}
            textAnchor="middle"
            fill={Y}
            fontSize={8}
          ...
          {showRes &&
            pts.map((p, i) => (
              <line
                ...
                y1={scy(p.y)}
                x2={scx(p.x)}
                y2={scy(clY(line.slope * p.x + line.intercept))}
                stroke="#f97316"
                strokeWidth={1.5}
                strokeDasharray="3 2"
                opacity={0.65}
          ...
          <line
            x1={scx(predictX)}
            y1={scy(Math.max(0, Math.min(YMAX, pyC)))}
            x2={scx(predictX)}
            y2={CH - PB}
            stroke={Y}
            strokeWidth={1}
      ...
      >
        <div style={{ display: "flex", gap: 0, flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              fontSize: 11,
              fontWeight: 600,
              cursor: pts.length < 2 ? "not-allowed" : "pointer",
              opacity: pts.length < 2 ? 0.35 : 1,
              lineHeight: 1,
            }}
          >
            Learn
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${T.accent}`,
              borderLeft: `1px solid ${T.accent}40`,
        ...
        <div
          style={{ width: 1, height: 14, background: T.border, flexShrink: 0, margin: "0 2px" }}
        />
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <span style={{ whiteSpace: "nowrap" }}>
            Σ{" "}
            <span style={{ color: showRes ? "#f97316" : T.textDim, fontWeight: 600 }}>
              ${sigma.toFixed(0)}
            </span>
          </span>
          <span style={{ whiteSpace: "nowrap" }}>
          ...
          >
            <span style={{ color: Y }}>converged ✓</span>
            {"  "}y = <span style={{ color: Y }}>{line.slope.toFixed(1)}</span>x{" "}
            {line.intercept >= 0 ? "+" : "−"}{" "}
...
function ContextChip({ word, id, morphDone }: { word: string; id: string; morphDone: boolean }) {
  return (
    <motion.span
      layout
      layoutId={`dot-${id}`}
...
function TokenPredict({ pts }: { pts: Pt[] }) {
  ...
  for (let i = 0; i < count; i++)
    chronological.push({ kind: "ctx", id: String(pts[i].id), word: wordAt(i) });
  for (let i = 0; i < predicted; i++)
    chronological.push({ kind: "gen", idx: count + i, word: wordAt(count + i) });
  ...
  return (
    ...
    >
      <motion.p
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: CHIP_DELAY + CHIP_DURATION - 0.2, duration: 0.3 }}
        style={{
          fontSize: 12,
          color: T.textDim,
          fontFamily: "Inter,sans-serif",
      ...
      >
        Each point became a{" "}
        <span style={{ color: Y, fontFamily: "JetBrains Mono,monospace", fontSize: 11 }}>
          token
        </span>
        . The{" "}
        <span
      ...
      <motion.button
        ...
        transition={{ delay: BUTTON_DELAY, duration: 0.4 }}
        onClick={predict}
        style={{
          padding: "9px 30px",
          borderRadius: 22,
      ...
      >
        Regression predicts a number from inputs. LLMs do the same — but predict the next{" "}
        <em>token</em>.
      </motion.p>
    </motion.div>
  );
...
export default function LinearRegression({ isActive: _isActive }: SlideProps) {
  ...
  const [svgRelRect, setSvgRelRect] = useState<{
    left: number;
    top: number;
    width: number;
    height: number;
  ...
  return (
    ...
    >
        ...
        <AnimatePresence mode="sync">
          ...
          {part === "learn" && (
            <LearnChart
              ...
              setLine={setLine}
              predictX={predictX}
              setPredictX={setPredictX}
            />
          )}
          {part === "tokens" && <TokenPredict key="tokens" pts={pts} />}
        </AnimatePresence>
      ...
      <div
        style={{ display: "flex", gap: 6, justifyContent: "center", paddingTop: 10, flexShrink: 0 }}
      >
        {(
          [
