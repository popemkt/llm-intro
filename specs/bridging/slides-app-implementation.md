...
| Manual block layout actions | `arrange-manual-blocks`, `duplicate-manual-blocks`, and `move-manual-block-layer` mirror visual editor alignment, distribution, match width/height/size, duplication, and layer ordering behavior for agents | Vitest/API |
