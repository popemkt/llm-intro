...
function buildHtmlFromIndex(): string {
  ...
  html = html.replace(
    /<\/head>/,
    "<style>html,body,#root{height:100%;overflow:hidden;margin:0}*{box-sizing:border-box}</style></head>",
  );
  return html;
}
    ...
    try {
      ...
      if (Array.isArray(slideIds) && slideIds.length > 0) {
        ...
        const retainedGroupIds = new Set(
          slides.map((s) => s.group_id).filter((id): id is number => id != null),
        );
      ...
      const builtHtmlPath = fs.existsSync(workspace.distHtmlPath)
        ? workspace.distHtmlPath
        : fs.existsSync(workspace.nestedDistHtmlPath)
          ? workspace.nestedDistHtmlPath
          : null;
