...
function findClosest(vec: number[], exclude: string[] = []) {
  return WORD_LIST.filter((w) => !exclude.includes(w))
...
function ExploreTab({
  ...
  const simList = WORD_LIST.filter((x) => x !== selected)
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      <div style={{ fontSize: "12px", color: T.textDim, lineHeight: 1.6 }}>
        AI gives every word a hidden <span style={{ color: T.accent }}>recipe</span> — a list of
        numbers, one per dimension. Pick any word to see its fingerprint.
      </div>
...
function CompareTab({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      <div style={{ fontSize: "12px", color: T.textDim, lineHeight: 1.6 }}>
        Similar meanings have <span style={{ color: T.accent }}>similar-shaped fingerprints</span>.
              ...
              >
                ...
                >
                  {diff < 0.5
                    ? "identical ✓"
                    : diff > 4
                      ? `big diff (${diff.toFixed(1)})`
                      : `slight diff (${diff.toFixed(1)})`}
                </span>
              </div>
              <div style={{ display: "flex", gap: "4px", marginBottom: "3px" }}>
                {[
                  { vec: vecA, color: wA.color },
                  { vec: vecB, color: wB.color },
                ].map((x, xi) => (
                  <div
                    key={xi}
                    style={{
...
function MagicTab({
  ...
  mathB: string;
  mathC: string;
  setMathA: (v: string) => void;
  setMathB: (v: string) => void;
  setMathC: (v: string) => void;
}) {
  const { mathVec, closest } = useMemo(() => {
    const a = getVec(mathA),
      b = getVec(mathB),
      c = getVec(mathC);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      <div style={{ fontSize: "12px", color: T.textDim, lineHeight: 1.6 }}>
        Because words are numbers, we can do <span style={{ color: T.accent }}>arithmetic</span> on
        them — subtract one fingerprint, add another, land on a different word.
      </div>
      ...
      >
        ...
        <span style={{ fontSize: "22px", color: "#FB7185", fontWeight: 300, lineHeight: 1 }}>
          −
        </span>
        <WordSelect value={mathB} onChange={setMathB} />
        <span style={{ fontSize: "22px", color: "#4ADE80", fontWeight: 300, lineHeight: 1 }}>
          +
        </span>
      ...
      <div
        style={{
          ...card,
          textAlign: "center",
          borderColor: `${WORDS[rw].color}44`,
          background: `${WORDS[rw].color}09`,
        }}
      >
        <div
          style={{
      ...
      <div style={{ ...card, display: "flex", gap: "12px", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "1 1 150px" }}>
          ...
          {DIMS.map((dim) => {
            const a = WORDS[mathA][dim.key],
              b = WORDS[mathB][dim.key],
              c = WORDS[mathC][dim.key];
