...
const layoutBuilders: Record<NormalSlideLayout, LayoutBuilder> = {
  ...
  "two-column": (input, title, label) => [
    ...
    text("left", bulletMarkdown(input.leftBullets ?? input.bullets ?? []), {
      x: 8,
      y: 40,
      w: 38,
      h: 42,
    ...
    input.visualDescription
      ...
      : text("right", bulletMarkdown(input.rightBullets ?? ["Supporting detail", "Second point"]), {
          x: 54,
          y: 40,
          w: 36,
          h: 42,
  ...
  quote: (input, title, _label, subtitle) => [
    ...
    text("attribution", input.attribution?.trim() || subtitle || "Attribution", {
      x: 8,
      y: 66,
      w: 46,
      h: 8,
  ...
  closing: (_input, title, label, subtitle) => [
    ...
    text("subtitle", subtitle || "Add the call to action or contact details", {
      x: 8,
      y: 65,
      w: 58,
      h: 10,
