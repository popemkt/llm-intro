...
const NODES = {
  ...
  manual: {
    ...
    w: 150,
    h: 44,
    label: "Keep using manually",
    sub: "works great as-is",
    accent: false,
  },
  auto: {
    x: 430,
    ...
    >
      ...
      {n.sub && (
        <text
          ...
          textAnchor="middle"
          dominantBaseline="middle"
          fill={active ? (n.accent ? `${T.bg}99` : T.textDim) : T.textDim}
          fontSize={8.5}
...
export default function Appendix({ isActive }: SlideProps) {
  ...
  return (
    ...
    >
      ...
      {/* Main content */}
      <div
        style={{ flex: 1, display: "flex", gap: 24, alignItems: "flex-start", overflow: "hidden" }}
      >
        {/* SVG flow */}
        <svg
          viewBox={`0 0 ${W} ${H}`}
          style={{ flex: "0 0 auto", width: "60%", maxWidth: W }}
        ...
        >
          ...
          {/* Arrows */}
          <FlowArrow
            d={`M${cx(NODES.dev)} ${cb(NODES.dev)} L${cx(NODES.stable)} ${ct(NODES.stable)}`}
            delay={0.5}
            isActive={isActive}
            accent
            markerEnd="url(#app-a)"
          />
          <FlowArrow
            d={`M${cx(NODES.stable)} ${cb(NODES.stable)} C${cx(NODES.stable)} ${cb(NODES.stable) + 30} ${cx(NODES.manual)} ${ct(NODES.manual) - 30} ${cx(NODES.manual)} ${ct(NODES.manual)}`}
            delay={0.8}
            isActive={isActive}
            markerEnd="url(#app)"
          />
          <FlowArrow
            d={`M${cx(NODES.stable)} ${cb(NODES.stable)} C${cx(NODES.stable)} ${cb(NODES.stable) + 30} ${cx(NODES.auto)} ${ct(NODES.auto) - 30} ${cx(NODES.auto)} ${ct(NODES.auto)}`}
            delay={0.8}
            isActive={isActive}
            markerEnd="url(#app)"
          />
          <FlowArrow
            d={`M${cx(NODES.auto)} ${cb(NODES.auto)} L${cx(NODES.n8n)} ${ct(NODES.n8n)}`}
            delay={1.1}
            isActive={isActive}
            accent
            markerEnd="url(#app-a)"
          ...
          {[
            {
              x: cx(NODES.dev) + 10,
              y: (cb(NODES.dev) + ct(NODES.stable)) / 2,
              t: "iterate & test",
            },
            {
              x: (cx(NODES.stable) + cx(NODES.manual)) / 2 - 20,
              y: (cb(NODES.stable) + ct(NODES.manual)) / 2 + 10,
              t: "if manual is fine",
            },
            {
              x: (cx(NODES.stable) + cx(NODES.auto)) / 2 + 20,
              y: (cb(NODES.stable) + ct(NODES.auto)) / 2 + 10,
              t: "if repetitive",
            },
