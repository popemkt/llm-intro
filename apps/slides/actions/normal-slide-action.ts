...
export function createNormalSlideAction(slidesService: SlidesService) {
  return defineAction({
    description: 'Create a themeable DB-backed slide using one of the standard normal slide layouts.',
    schema: z.object({
    ...
    publicAgent: {
      ...
      description: 'Create a standard, themeable slide layout: title, section, bullets, two-column, quote, metrics, or closing.',
    },
    run: ({ pid, layout, ...input }) => slidesService.create(pid, {
